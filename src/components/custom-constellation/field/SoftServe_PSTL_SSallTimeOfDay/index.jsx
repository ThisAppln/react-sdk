...
const SoftServePstlSSallTimeOfDay = props => {
  const {
    getPConnect,
    value,
    validatemessage,
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'STACKED_LARGE_VAL' || displayMode === 'DISPLAY_ONLY') {
    ...
    switch (displayMode) {
      case 'DISPLAY_ONLY': {
        return ( <StyledSoftServePstlSSallTimeOfDayWrapper> displayComp </StyledSoftServePstlSSallTimeOfDayWrapper>);
      }
      case 'LABELS_LEFT': {
        return (
...
export default SoftServePstlSSallTimeOfDay;
